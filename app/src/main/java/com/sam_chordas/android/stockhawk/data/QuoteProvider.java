...
public class QuoteProvider {
    ...
    public static class H_QUOTES {
        @ContentUri(
                path = Path.H_QUOTES,
                type = "vnd.android.cursor.dir/HQuotes"
        )
        public static final Uri CONTENT_URI = buildUri(Path.H_QUOTES);

        @InexactContentUri(
                name = "QUOTE_ID",
                path = Path.H_QUOTES + "/*",
                type = "vnd.android.cursor.item/HQuotes",
                whereColumn = QuoteColumns.SYMBOL,
                pathSegment = 1
        )
        public static Uri withSymbol(String symbol) {
